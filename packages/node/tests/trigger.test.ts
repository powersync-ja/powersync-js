  ...
  databaseTest('Should accept a lock context', { timeout: 100_000 }, async ({ database }) => {
    ...
    const setupWatch = () =>
      ...
      database.triggers.trackTableDiff({
        ...
        onChange: async (context) => {
          ...
          console.log('New todos fetched:', newTodos.length);
          todos.push(...newTodos);
        },
        hooks: {
          beforeCreate: async (lockContext) => {
            ...
            console.log('Current todos:', currentTodos.length);
          }
        }
      });
    ...
    await vi.waitFor(
      async () => {
        ...
        console.log('Expected:', await database.get(`SELECT COUNT(*) FROM todos WHERE list_id = ?`, [firstList.id]));
        console.log('Expected:', await database.get(`SELECT COUNT(*) FROM todos WHERE list_id = ?`, [firstList.id]));
