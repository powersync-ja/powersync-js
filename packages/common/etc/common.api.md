  ...
  implements WatchedQuery<Data, Settings>
{
  ...
  protected iterateAsyncListenersWithError(
...
export class AttachmentService {
  ...
  watchActiveAttachments(input?: { throttleMs?: number }): DifferentialWatchedQuery<AttachmentRecord>;
...
export class CrudBatch {
  constructor(crud: CrudEntry[], haveMore: boolean, complete: (writeCheckpoint?: string) => Promise<void>);
...
export class CrudTransaction extends CrudBatch {
  constructor(crud: CrudEntry[], complete: (checkpoint?: string) => Promise<void>, transactionId?: number | undefined);
...
//
// @internal
export function DBAdapterDefaultMixin<TBase extends new (...args: any[]) => ConnectionPool>(
  Base: TBase
): {
  new (...args: any[]): {
    readTransaction<T>(fn: (tx: Transaction) => Promise<T>, options?: DBLockOptions): Promise<T>;
...
export class GetAllQuery<RowType = unknown> implements WatchCompatibleQuery<RowType[]> {
  ...
  compile(): CompiledQuery;
  // (undocumented)
  execute(options: { db: AbstractPowerSyncDatabase }): Promise<RowType[]>;
  // (undocumented)
...
// @public
export type PendingStatementParameter =
  | 'Id'
  | {
      Column: string;
    }
...
export interface StreamingSyncImplementation
  extends BaseObserverInterface<StreamingSyncImplementationListener>, Disposable_2 {
...
export class TriggerManagerImpl implements TriggerManager {
  ...
  ): Promise<(options?: TriggerRemoveCallbackOptions) => Promise<void>>;
  // (undocumented)
...
export class UploadQueueStats {
  constructor(count: number, size?: number | null);
  count: number;
  size: number | null;
  // (undocumented)
  toString(): string;
