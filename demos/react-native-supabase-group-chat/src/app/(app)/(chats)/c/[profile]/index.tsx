...
export default function ChatsChatIndex() {
  ...
  const profiles = usePowerSyncWatchedQuery(
    "SELECT id, name, handle, demo FROM profiles WHERE id = ?",
    [profileId]
  );
  ...
  useEffect(() => {
    ...
    if (user?.id && profileId) {
      if (!draftId) {
        ...
        powerSync.execute("UPDATE messages SET content = ? WHERE id = ?", [
          message,
          draftId,
        ]);
      }
  ...
  async function handleInputSubmit() {
    await powerSync.execute(
      "UPDATE messages SET content = ?, sent_at = datetime() WHERE id = ?",
      [message, draftId]
    );
    ...
    if (profile.demo === 1) {
      ...
      setTimeout(async () => {
        await powerSync.execute(
          "UPDATE messages SET sent_at = datetime(), content = ? WHERE id = ?",
          [message, messageId]
        );
      }, 2000);
    }
    ...
    <>
      ...
      <YStack fullscreen>
        ...
        <YStack padding="$3" gap="$3">
          {profile.demo === 1 && (
            <Button onPress={handleDemoMessage}>Receive demo message</Button>
          )}
          <Input
