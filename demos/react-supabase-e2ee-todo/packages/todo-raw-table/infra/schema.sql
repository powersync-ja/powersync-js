...
-- Helpful indexes
-- Allow multiple lists per provider per user (drop unique constraint)
drop index if exists idx_e2ee_keys_user_provider;
create index if not exists idx_e2ee_keys_user_provider on public.e2ee_keys(user_id, provider);
